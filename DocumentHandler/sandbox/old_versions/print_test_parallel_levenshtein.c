...
Data (última atualização): 06/04/2014
...
int levenshtein(char* s, int len_s, char* t, int len_t)
{
	...
	/*initalization vertical semaphore*/
	sem_init(&vsem, 0, 0);
...
void printMatrix(unsigned int** matrix, int n, int m)
{
	...
	int k = 0;
	char* s;
	s = (char*) malloc(sizeof(char) * n * m *2); //alloc space to formated matrix like (m[3][3]): "a00 a01 a02\na10 a11 a12\na20 a21 a22\0"
	for (i = 0; i < n; i++)
	{
		for (j = 0; j < m; j++)
		{
			s[k] = matrix[i][j] + 48; //put the value in the cell
			k++;
			if (j < m-1) // put the space
				s[k] = ' '; //put the space
			else
				s[k] = '\n'; //put the line break
			k++;
